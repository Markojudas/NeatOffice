...
namespace NeatOffice
{
    ...
    class GraphAlgorithms
    {
        ...
        internal void GetMST(string fileName)
        {
            ...
            while (queue.Count > 0)
            {
                p.Value = (int)Math.Round(100 * (1 - queue.Count / (double)vertexCount));
        ...
        internal void ReadGraphFromCSVFile(string graphFileName)
        {
            ...
            {
                p.Value = (int)Math.Round(100 * i / (double)lines.Length);
        ...
        internal void Dijkstra(string fileName)
        {
            int source = 0;
            int n = Math.Min(Math.Max(25, fileName.Length / 2), fileName.Length);
            l.Text = "running Dijkstra's to find shortest paths from source " + source + " for graph in ...";
